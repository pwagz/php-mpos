...
$(function () {
      ...
      Morris.Area({
        ...
        data: blockdata,
        xkey: 'blockHeight',
        ykeys : ['actShares', {if $USEBLOCKAVERAGE}'avgShares', {/if}{if $GLOBAL.config.payout_system == 'pplns'}'pplnsShares', {/if} 'estShares'],
        labels : ['Actual Shares', {if $USEBLOCKAVERAGE}'Average Shares', {/if}{if $GLOBAL.config.payout_system == 'pplns'}'PPLNS Shares',{/if} 'Expected Shares'],
        pointSize: 2,
        lineColors: ['#2D619C','#a2b3bf','#2D9C2F'],
